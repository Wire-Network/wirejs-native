...
const ripemd160 = rip.RIPEMD160.hash as (a: Uint8Array) => ArrayBuffer;
...
export const signedDecimalToBinary = (size: number, s: string): Uint8Array => {
    ...
    const result = decimalToBinary(size, modifiedString);
 ...
 * @param minDigits 0-pad result to this many digits
 */
export const signedBinaryToDecimal = (
    bignum: Uint8Array,
    minDigits = 1
): string => {
...
export const digestSuffixRipemd160 = (
    data: Uint8Array,
    suffix: string
): ArrayBuffer => {
...
export const stringToPublicKey = (s: string): Key => {
    ...
    if (s.substr(0, 3) === 'SYS') {
        ...
        return stringToKey(s.substr(7), KeyType.em, publicKeyDataSize, 'EM');
    } else {
...
export const publicKeyToLegacyString = (key: Key): string => {
    if (
        (key.type === KeyType.k1 || key.type === KeyType.em) &&
        key.data.length === publicKeyDataSize
    ) {
...
export const stringToSignature = (s: string): Key => {
    ...
    if (s.substr(0, 7) === 'SIG_K1_') {
        ...
        return stringToKey(s.substr(7), KeyType.em, signatureDataSize, 'EM');
