...
export const supportedAbiVersion = (version: string): boolean => {
  return (
    version.startsWith("sysio::abi/1.") || version.startsWith("eosio::abi/1.")
  );
};
...
export const serializeAnyvar = (buffer: SerialBuffer, anyvar: Anyvar): void => {
  ...
  if (anyvar === null) {
    ...
    Object.keys(anyvar).length === 2 &&
    anyvar.hasOwnProperty("type") &&
    anyvar.hasOwnProperty("value")
  ) {
    [def, value] = [
      (anyvarDefs as any)[(anyvar as any).type] as AnyvarDef,
      (anyvar as any).value,
    ];
  } else {
...
export const deserializeAnyvar = (
  buffer: SerialBuffer,
  state?: SerializerState,
): any => {
...
export const deserializeAnyvarShort = (buffer: SerialBuffer): any => {
  return deserializeAnyvar(
    buffer,
    new SerializerState({ useShortForm: true } as any),
  );
};
...
export const deserializeAnyObject = (
  buffer: SerialBuffer,
  state?: SerializerState,
): any => {
  const len = buffer.getVaruint32();
...
export const deserializeAnyArray = (
  buffer: SerialBuffer,
  state?: SerializerState,
): any[] => {
  const len = buffer.getVaruint32();
