...
export class JsonRpc implements AuthorityProvider, AbiProvider {
  ...
  public async get_currency_stats(
    code: string,
    symbol: string,
  ...
  public async get_raw_code_and_abi(
    accountName: string,
  ...
  public async push_ro_transaction(
    { signatures, compression = 0, serializedTransaction }: PushTransactionArgs,
    returnFailureTraces: boolean = false,
  ...
  public async push_transactions(
  ...
  public async db_size_get(): Promise<DBSizeGetResult> {
    return await this.fetch("/v1/db_size/get", {});
  }

  /** Raw call to `/v1/trace_api/get_block` */
  public async trace_get_block(
    block_num: number,
  ): Promise<TraceApiGetBlockResult> {
  ...
  public async history_get_transaction(
    id: string,
  ...
  public async history_get_key_accounts(
    publicKey: string,
  ): Promise<GetKeyAccountsResult> {
  ...
  public async history_get_controlled_accounts(
